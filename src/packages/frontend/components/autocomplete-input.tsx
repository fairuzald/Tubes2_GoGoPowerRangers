  ...
  return (
    <div className="flex flex-col flex-1 relative">
      ...
      {isOpen && data && data.length > 0 && (
        <ul
          ref={dropdownRef}
          className="rounded-lg absolute top-16 bg-white text-black max-h-[175px] lg:max-h-[300px] z-10 custom-scrollbar overflow-y-auto overflow-x-hidden w-full"
          role="listbox"
          ...
          {data.map((item) => (
            ...
            >
              ...
              <Image
                ...
                alt={item.title}
                sizes="80px"
                className="aspect-square w-14 lg:w-20 object-cover rounded-lg flex-shrink-0"
              />
              {/* Text Content */}
              <div className="flex flex-col gap-2">
                <h3 className="text-base lg:text-lg font-bold">{item.title}</h3>
                <p className="text-xs lg:text-sm line-clamp-3">
                  {item.description}
                </p>
              </div>
            </li>
