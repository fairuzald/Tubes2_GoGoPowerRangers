...
func IDSHadlersBackup(source string, destination string, maxDepth int) ([][]string, int, error) {
	if source == destination {
		...
		for depth <= maxDepth {
			...
			resultPaths, counter, err := DFSHelperBackup(source, destination, depth)
...
func DFSHelperBackup(source string, destination string, maxDepth int) ([][]string, int, error) {
	stack := [][]string{{source}}
	paths := [][]string{}
	counter := 0
...
func IDSHTTPHandlerBackup(c *gin.Context) {
	type ReqBody struct {
