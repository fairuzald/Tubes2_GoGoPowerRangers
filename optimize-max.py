...
def bfs(source, destination, max_depth=6):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        while queue and len(path) < max_depth and not found:
            queue_same_depth = []
            while queue:
                queue_same_depth.append(queue.pop(0))

            futures = []
            for current_url, paths in queue_same_depth:
                if current_url == destination:
                    ...
                    futures.append(executor.submit(scraping_handler, current_url))
                    ...
                    with ThreadPoolExecutor(max_workers=5) as inner_executor:
                        for link in links:
                            if (link not in visited or not visited[link]) and len(paths) < max_depth:
                                ...
                                inner_executor.submit(queue.append, (link, paths + [link]))
